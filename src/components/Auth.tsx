    ...
    <div className="flex min-h-[100dvh] items-center justify-center p-6 bg-[#030303] relative overflow-hidden font-sans">
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }} className="w-full max-w-[320px] z-10">
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-semibold tracking-tight text-white mb-1">{resetEmailSent ? "Check Your Email" : "Reset Password"}</h1>
          <p className="text-[10px] text-zinc-500 font-medium uppercase tracking-widest">{resetEmailSent ? "Recovery link dispatched" : "Enter your email to recover"}</p>
        </div>
          ...
          {resetEmailSent ? (
            <CardContent className="p-6 md:p-8 space-y-5">
              <div className="text-center space-y-4">
                ...
                <p className="text-sm text-zinc-400">Password reset link has been sent to <span className="text-white font-medium">{email}</span></p>
                <p className="text-xs text-zinc-500">Check your inbox and follow the instructions to reset your password.</p>
                <div className="pt-4 space-y-3">
                  <Button 
                    onClick={() => { setResetEmailSent(false); }}
                    className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-all text-xs uppercase tracking-[0.2em]"
                  >
                    Send Another Link
                  </Button>
                  <button 
                    onClick={() => { setShowForgotPassword(false); setResetEmailSent(false); setEmail(""); }} 
                    className="w-full text-[10px] font-medium text-zinc-500 hover:text-white transition-colors uppercase tracking-widest py-2"
                  >
                    Back to Sign In
                  </button>
                </div>
            ...
            <form onSubmit={handleForgotPassword}>
              <CardContent className="p-6 md:p-8 space-y-5">
                <div className="space-y-2">
                  <Label htmlFor="resetEmail" className="text-[10px] font-semibold text-zinc-400 ml-1 uppercase tracking-widest">Email Address *</Label>
                  <div className="relative group">
                    <Input id="resetEmail" type="email" placeholder="name@example.com" value={email} onChange={(e) => setEmail(e.target.value)} required className="bg-white/[0.03] border-white/10 h-11 rounded-xl px-4 focus:ring-indigo-500/20 focus:border-indigo-500/40 transition-all text-white placeholder:text-zinc-600 text-xs" />
                ...
                <div className="pt-2">
                  <Button type="submit" className="w-full h-12 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold transition-all shadow-[0_0_25px_rgba(79,70,229,0.3)] disabled:opacity-30 text-xs uppercase tracking-[0.2em]" disabled={loading}>
                    {loading ? (
                      <div className="flex items-center gap-2"><div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin" /><span>Sending...</span></div>
                    ) : (
                      <div className="flex items-center justify-center gap-2"><span>Send Reset Link</span><ChevronRight className="w-4 h-4" /></div>
                    )}
                  </Button>
                </div>
              </CardContent>
              <CardFooter className="px-6 md:px-8 pb-6 flex justify-center border-t border-white/5 bg-white/[0.01]">
                <button type="button" onClick={() => { setShowForgotPassword(false); setEmail(""); }} className="text-[10px] font-medium text-zinc-500 hover:text-white transition-colors flex items-center gap-2 uppercase tracking-widest">
                  ...
                  <div className="space-y-2">
                    ...
                    {!isSignUp && (
                      <div className="text-right pt-1">
                        <button type="button" onClick={() => setShowForgotPassword(true)} className="text-[9px] font-medium text-indigo-400 hover:text-indigo-300 transition-colors uppercase tracking-widest">
                          Forgot Password?
                        </button>
                      </div>
                    )}
